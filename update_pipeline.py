...
# ── Step 1 — Fetch temperature from Open-Meteo ───────────────────────────
def fetch_temperature(date_str):
    ...
    try:
        ...
        data      = response.json()
        temp_dict = dict(zip(data['daily']['time'],
                             data['daily']['temperature_2m_max']))
        temp = temp_dict.get(date_str, np.nan)
        print(f"✅ Temperature for {date_str}: {temp}°C")
        return temp
    except Exception as e:
        print(f"⚠️ Weather API failed: {e} — using NaN")
        return np.nan

# ── Step 2 — Fetch agmarknet data ────────────────────────────────────────
def fetch_agmarknet(date_str):
    ...
    if response.status_code != 200:
        print(f"❌ Agmarknet failed for {date_str}: {response.status_code}")
        return None
    ...
    print(f"✅ Agmarknet fetched {len(df)} rows for {date_str}")
    return df

# ── Step 3 — Clean and format ─────────────────────────────────────────────
def clean_new_data(raw_df, temp_value=np.nan):
    df = raw_df.copy()
    df = df.rename(columns={
        ...
        'Arrival Quantity': 'arrival_qty_mt',
        'Arrival Date'    : 'date'
    })
    drop_cols = ['State','Commodity Group','Commodity',
...
def append_to_csv(new_df, data_path):
    ...
    combined = combined.sort_values(
        ['district','market','variety','grade','date']
...
def recompute_features(df):
    ...
    df['state_price_lag1']   = (df.groupby('date', observed=True)['modal_price']
                                   .transform('mean').shift(1))
    df['state_arrival_lag1'] = (df.groupby('date', observed=True)['arrival_qty_mt']
                                   .transform('mean').shift(1))
    ...
    df['days_since_season_start'] = df.apply(
        ...
        if row['is_sharbati']
        else (row['date'] - pd.Timestamp(f"{row['date'].year}-03-01")).days,
        axis=1)

    df['price_above_msp']      = df['price_lag1'] - df['msp']
    df['target_price_t1']      = (df.groupby(group_cols, observed=True)['modal_price']
                                     .shift(-1))
...
if __name__ == "__main__":
    ...
    # Fetch temperature first
    temp = fetch_temperature(yesterday)

    # Fetch mandi data
    raw = fetch_agmarknet(yesterday)

    if raw is not None:
        cleaned  = clean_new_data(raw, temp_value=temp)
